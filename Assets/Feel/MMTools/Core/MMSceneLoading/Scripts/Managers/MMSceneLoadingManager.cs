...
namespace MoreMountains.Tools
{
    ...
    public class MMSceneLoadingManager : MonoBehaviour
    {
        ...
        // ★ NEW: CanvasGroup to fade the loading text (auto-added at runtime if left null)
        ...
        // ★ NEW: explicit references for your hierarchy (safe to leave null; we’ll auto-find)
#if MM_UI
        [SerializeField] private Slider _progressSlider;         // assign your "Slider" here
#endif
        ...
        protected virtual void Start()
        {
...
#if MM_UI
            // ★ NEW: try to auto-find Slider and sliderBluefill if not assigned
            ...
            // ★ NEW: cache text value and ensure we have a CanvasGroup to fade the text
            ...
            if (LoadingTextGroup != null)
            {
                LoadingTextGroup.alpha = 1f; // ensure visible at start
            }
#endif
        ...
        protected virtual void Update()
        {
            Time.timeScale = 1f;
#if MM_UI
            // ★ CHANGED: drive Slider.value primarily
            float current = (_progressSlider != null) ? _progressSlider.value : (_progressBarImage != null ? _progressBarImage.fillAmount : 0f);
            float next = MMMaths.Approach(current, _fillTarget, Time.deltaTime * ProgressBarSpeed);

            if (_progressSlider != null)
            {
                _progressSlider.value = next;   // drives the Slider (recommended)
            }
#endif
        }
        ...
        protected virtual IEnumerator LoadAsynchronously()
        {
...
#if MM_UI
            {
                // ★ CHANGED: consider Slider.value first, else fall back to image fillAmount
                while (true)
                {
                    bool reached;
                    if (_progressSlider != null)
                    ...
                    {
                        reached = true; // nothing to animate, avoid infinite loop
                    }
            ...
            // the load is now complete, we replace the progress bar with the complete animation
            LoadingComplete();
...
#if MM_UI
            // ★ CHANGED: initialize via Slider if present; fall back to image
            if (_progressSlider != null) { _progressSlider.value = 0f; }
            if (LoadingText != null) { LoadingText.text = _loadingTextValue; }
            if (LoadingTextGroup != null) { LoadingTextGroup.alpha = 1f; } // ★ NEW: make sure text starts visible
#endif
        }
        ...
        protected virtual void LoadingComplete()
        {
            ...
            // ★ NEW: fade out the loading text too
            if (LoadingTextGroup != null)
            {
